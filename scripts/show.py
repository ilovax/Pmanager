...
from crypt_functions import decrypt

def get_options(args, account, email, help_msgs):

	# If user didn't supply options 
	if len(args) == 0 :
		logging.warning("Usage : show.py -a <account_name> -e <email_address>")
	...
	for opt, arg in opts:
		if opt in ("-h","--help"):
			logging.info("Usage : show.py -a <account_name> -e <email_address>")
...
if __name__ == "__main__":
	...
	# Finding the crypted password
	accounts = Account.objects(email=email, name=account)
	for acc in accounts:
		password = decrypt(acc.password,acc.iv)
	
	logging.info(f"The {account} password for {email} is {password}")
